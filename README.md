...
const engine = createLayoutEngine({
    // Uses the default prefix "ly-sys"
...
> [!IMPORTANT]
> **Custom Prefixes and PostCSS Configuration**
> 
...
>    };
>    ```
> 2. Replace the `@import` with the `@ly-sys-layout;` directive in your main CSS file:
>    ```css
>    @import "tailwindcss";
>    @ly-sys-layout;
>    ```
> 
> If you do not configure a custom prefix (meaning you keep the default `"ly-sys"`), the PostCSS plugin is **not**
> required, and you can simply import `@ly-sys/layout/styles.css` directly.
...
For remotes/micro-frontends registering critical or deferred CSS, you can modularize your styles in real `.css` files
and load them dynamically using `?inline` imports instead of raw JavaScript strings.
